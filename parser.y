                ...
                | VOID ID MK_LPAREN param_list MK_RPAREN MK_LBRACE block MK_RBRACE      
                    {
                        ...
                        $$ = makeDeclNode(FUNCTION_DECL);
                        AST_NODE* parameterList = Allocate(PARAM_LIST_NODE);
                        AST_NODE* type_null = makeIDNode("null", NORMAL_ID);
                        makeChild(parameterList, $4);
                        makeFamily($$, 4, type_null, makeIDNode($2, NORMAL_ID), parameterList, $7);
                    }
                ...
                | VOID ID MK_LPAREN  MK_RPAREN MK_LBRACE block MK_RBRACE 
                    {
                        ...
                        AST_NODE* emptyParameterList = Allocate(PARAM_LIST_NODE);
                        AST_NODE* type_null = makeIDNode("null", NORMAL_ID);
                        makeFamily($$, 4, type_null, makeIDNode($2, NORMAL_ID), emptyParameterList, $6);
                    } 
                ;
            ...
            | type ID dim_fn 
                {
                    /*TODO*/
                    $$ = makeDeclNode(FUNCTION_PARAMETER_DECL);
                    AST_NODE* func_id = makeIDNode($2, ARRAY_ID);
                    makeChild(func_id, $3);  /* child to "type" or "ID"? */
                    makeFamily($$, 2, $1, func_id);
                }
            ;
...
block           : decl_list stmt_list 
                    {
                        ...
                        makeChild($$, makeChild(Allocate(STMT_LIST_NODE), $2));
                    }
                | stmt_list  
                    {
                        $$ = Allocate(BLOCK_NODE);
...
id_list		: ID 
            ...
            | id_list MK_COMMA ID dim_decl
                {
                    /*TODO*/
                    AST_NODE* var = makeIDNode($3, ARRAY_ID);
                    makeFamily($$, 2, var, $4);
                }
            | ID dim_decl
                {
                    /*TODO*/
                    AST_NODE* var = makeIDNode($1, ARRAY_ID);
                    makeSibling(var, $2);
                }
		;
...
factor		: MK_LPAREN relop_expr MK_RPAREN
            ...
            | OP_MINUS MK_LPAREN relop_expr MK_RPAREN
                {
                    AST_NODE* op_neg = makeExprNode(UNARY_OPERATION, UNARY_OP_NEGATIVE);
                    makeChild(op_neg, $3);
                    makeChild($$, op_neg);
                }
            ...
            | OP_MINUS ID MK_LPAREN relop_expr_list MK_RPAREN
                {
                    ...
                    makeFamily($$, 2, makeIDNode($2, NORMAL_ID), $4);
                }
